...
use ipxcs_core::{Tensor as CoreTensor, apply_ops, argmax, softmax, topk_indices, topk_values};
use ipxml_schema::{DecodeSpec, InputSpec, IpxmlApp, ModelInputBinding, OutputSpec, TensorSpec};
use ipxml_ui_core::{
    InputValue, OutputValue, UiBackend, UiContext, find_input, find_output, input_value_for_spec,
    output_value_for_spec,
};
...
pub struct ModelEntry {
    ...
    pub rules: Option<Vec<ipxml_schema::RuleSpec>>,
}
...
fn render_component(ui: &mut egui::Ui, app: &IpxmlApp, state: &mut AppState, id: &str) {
    if let Some(input) = find_input(app, id) {
        ...
        ui.colored_label(egui::Color32::YELLOW, format!("Unknown component id: {id}"));
    }
}

fn render_input(ui: &mut egui::Ui, state: &mut AppState, spec: &InputSpec) {
    if !is_visible(spec.when.as_deref(), spec.rules.as_deref(), &state.inputs) {
        return;
    }
    let value = state
        .inputs
    ...
    card(ui, |ui| {
        match (spec.kind.trim().to_ascii_lowercase().as_str(), value) {
            ("number_list" | "number_vector" | "vector", InputValue::NumberList(values)) => {
                ui.label(&spec.label);
                render_number_list_editor(ui, spec, values);
            }
            ...
            ("checkbox", InputValue::Bool(current)) => {
                ui.checkbox(current, &spec.label);
            }
            ("multiple_choice", InputValue::Choice(current)) => {
                ui.label(&spec.label);
                let selected = current.clone();
                egui::ComboBox::from_id_source(format!("choice_{}", spec.id))
                    .selected_text(selected)
                    .show_ui(ui, |ui| {
                        if let Some(choices) = &spec.choices {
                            for choice in choices {
                                let value =
                                    choice.value.clone().unwrap_or_else(|| choice.id.clone());
                                ui.selectable_value(current, value, &choice.label);
                            }
                        }
                    });
            }
            ("multi_select", InputValue::Choices(current)) => {
                ui.label(&spec.label);
                if let Some(choices) = &spec.choices {
                    for choice in choices {
                        let value = choice.value.clone().unwrap_or_else(|| choice.id.clone());
                        let mut selected = current.contains(&value);
                        if ui.checkbox(&mut selected, &choice.label).changed() {
                            if selected {
            ...
            ("audio", InputValue::AudioPath(current))
            | ("video", InputValue::VideoPath(current)) => {
                ui.label(&spec.label);
                ui.horizontal(|ui| {
                    if ui
                        .add_sized(
            ...
            (_, InputValue::NumberList(values)) => {
                ui.label(format!("{} ({})", spec.label, spec.kind));
                render_number_list_editor(ui, spec, values);
            }
            _ => {
...
fn render_number_list_editor(ui: &mut egui::Ui, spec: &InputSpec, values: &mut Vec<f64>) {
    if let Some(fields) = &spec.fields {
        ...
        for (idx, field) in fields.iter().enumerate() {
            ui.horizontal(|ui| {
                ...
                ui.add(egui::DragValue::new(&mut values[idx]).speed(0.05));
            });
        }
        return;
    }
...
fn render_output(ui: &mut egui::Ui, state: &mut AppState, spec: &OutputSpec) {
    ...
    card(ui, |ui| {
        match (spec.kind.trim().to_ascii_lowercase().as_str(), value) {
            ...
            ("audio", OutputValue::AudioPath(current))
            | ("video", OutputValue::VideoPath(current)) => {
                ui.label(&spec.label);
                ui.label(current.as_str());
            }
...
fn card(ui: &mut egui::Ui, content: impl FnOnce(&mut egui::Ui)) {
    let frame = egui::Frame::none()
        ...
        .stroke(egui::Stroke::new(
            1.0,
            egui::Color32::from_rgb(220, 226, 235),
        ))
...
fn apply_style(ctx: &egui::Context) {
    ...
    visuals.widgets.inactive.bg_stroke =
        egui::Stroke::new(1.0, egui::Color32::from_rgb(220, 226, 235));
...
fn set_output_status(value: &mut OutputValue, status: &str) {
    match value {
        ...
        OutputValue::AudioPath(current) | OutputValue::VideoPath(current) => {
            current.clear();
        }
    }
}
...
impl PipelineRunner {
    ...
    pub fn run(
        ...
        for model in &mut self.models {
            if !should_run_model(model, inputs) {
                continue;
            }
        ...
        for spec in &app.outputs {
            ...
            if kind == "audio" {
                if let Some(InputValue::AudioPath(path)) = inputs.get(source) {
                    result.insert(spec.id.clone(), OutputValue::AudioPath(path.clone()));
                    continue;
                }
            }
            if kind == "video" {
                if let Some(InputValue::VideoPath(path)) = inputs.get(source) {
                    result.insert(spec.id.clone(), OutputValue::VideoPath(path.clone()));
                    continue;
                }
            }
            ...
            if let Some(tensor) = model_outputs.get(&(model_id.clone(), source.to_string())) {
                ...
                let output_value = process_output_tensor(tensor.clone(), spec, labels, inputs)?;
...
fn build_model_inputs(
    ...
    for (input_name, source) in bindings {
        if let Some((model_id, output_name)) = parse_model_output_source(&source) {
            ...
            let tensor = build_input_tensor(meta, spec, value, inputs)?;
...
fn build_input_tensor(
    ...
    let array = match value {
        ...
        InputValue::NumberList(values) => number_list_to_array(values, &shape)?,
        InputValue::Bool(flag) => scalar_to_array(if *flag { 1.0 } else { 0.0 }, &shape),
        InputValue::Choice(choice) => {
            let parsed = choice.parse::<f32>().unwrap_or(0.0);
            scalar_to_array(parsed, &shape)
        }
        InputValue::Choices(choices) => {
            let parsed = choices
                .iter()
                .map(|v| v.parse::<f64>().unwrap_or(0.0))
                .collect::<Vec<_>>();
            number_list_to_array(&parsed, &shape)?
        }
        ...
        InputValue::AudioPath(path) => audio_to_array(path, &shape)?,
        InputValue::VideoPath(path) => video_to_array(path, spec, &shape)?,
    };

    let mut tensor = CoreTensor::new(array);
    if let Some(ops) = &spec.preprocess {
        let resolved = resolve_conditional_ops(ops, inputs);
        tensor = apply_ops(tensor, &resolved)?;
    }
...
fn number_list_to_array(values: &[f64], shape: &[usize]) -> Result<ArrayD<f32>> {
    if shape.is_empty() {
        let value = values.first().copied().unwrap_or(0.0) as f32;
        return Ok(ArrayD::from_elem(IxDyn(&[]), value));
    }
    ...
    for (idx, item) in values.iter().enumerate().take(size) {
        data[idx] = *item as f32;
    }
    ArrayD::from_shape_vec(IxDyn(shape), data).context("create tensor from numeric list")
}

fn audio_to_array(path: &str, shape: &[usize]) -> Result<ArrayD<f32>> {
    let mut reader = hound::WavReader::open(path).with_context(|| format!("open wav at {path}"))?;
    let mut samples = Vec::<f32>::new();
    for sample in reader.samples::<i16>() {
        let value = sample.unwrap_or(0) as f32 / i16::MAX as f32;
        samples.push(value);
    }
    if samples.is_empty() {
        samples.push(0.0);
    ...
    let mut data = vec![0.0_f32; size];
    for (idx, sample) in samples.into_iter().enumerate().take(size) {
        data[idx] = sample;
    }
    ArrayD::from_shape_vec(IxDyn(shape), data).context("create audio tensor")
}

fn video_to_array(path: &str, spec: &InputSpec, shape: &[usize]) -> Result<ArrayD<f32>> {
    // Phase-1 fallback: interpret video path as image frame source.
    image_to_array(path, spec, shape)
}

fn should_run_model(model: &ModelRunner, inputs: &HashMap<String, InputValue>) -> bool {
    if let Some(when) = &model.when {
        return eval_condition(when, inputs);
    }
    if let Some(rules) = &model.rules {
        for rule in rules {
            if eval_condition(&rule.if_expr, inputs) {
...
fn is_visible(
    ...
    if let Some(expr) = when {
        return eval_condition(expr, inputs);
    }
...
fn eval_condition(expr: &str, inputs: &HashMap<String, InputValue>) -> bool {
...
fn resolve_conditional_ops(
    ops: &[ipxml_schema::OpSpec],
    inputs: &HashMap<String, InputValue>,
) -> Vec<ipxml_schema::OpSpec> {
    let mut out = Vec::new();
    for op in ops {
        match op {
            ipxml_schema::OpSpec::ApplyIf {
                when,
                rules,
                then_ops,
                otherwise,
            } => {
                let mut should_then = true;
                if let Some(expr) = when {
                    should_then = eval_condition(expr, inputs);
                } else if let Some(rules) = rules {
                    for rule in rules {
                        if eval_condition(&rule.if_expr, inputs) {
                            should_then = rule.then.as_ref().and_then(|a| a.run).unwrap_or(true);
                            break;
                        } else if let Some(otherwise_action) = &rule.otherwise {
                            if let Some(run) = otherwise_action.run {
                                should_then = run;
                                break;
                            }
                        }
                ...
                if should_then {
                    out.extend(resolve_conditional_ops(then_ops, inputs));
                } else if let Some(otherwise_ops) = otherwise {
                    out.extend(resolve_conditional_ops(otherwise_ops, inputs));
                }
            }
            _ => out.push(op.clone()),
        }
...
fn tensor_to_scores(tensor: &CoreTensor, labels: &[String]) -> Option<Vec<(String, f32)>> {
    ...
    if values.len() != labels.len()
        && !(shape.len() == 2 && shape[0] == 1 && shape[1] == labels.len())
    {
        return None;
    }
